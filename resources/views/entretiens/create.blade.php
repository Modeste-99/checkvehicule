...
<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-2">Ajouter un entretien</h1>
    <p class="text-gray-600">Enregistrez un nouvel entretien pour un véhicule</p>
</div>

<div class="bg-white rounded-xl shadow-lg p-6 md:p-8 max-w-2xl">
    <form method="POST" action="{{ route('entretiens.store') }}" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="type" value="{{ old('type') }}" 
                ...
                placeholder="Ex: Vidange, Révision, Pneus..." required>
            @error('type')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="date" name="date_entretien" value="{{ old('date_entretien') }}" 
        ...
        <div>
            ...
            <textarea name="note" rows="4" 
                class="w-full px-4 py-3 text-gray-900 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all duration-200 @error('note') border-red-500 @enderror" 
                placeholder="Ajoutez des notes sur cet entretien...">{{ old('note') }}</textarea>
            @error('note')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
