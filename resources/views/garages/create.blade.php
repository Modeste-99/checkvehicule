...
@section('title', 'Ajouter un garage')

@section('content')

<div class="mb-6">
    <h1 class="text-3xl font-bold text-gray-900 mb-2">Ajouter un garage</h1>
    <p class="text-gray-600">Enregistrez un nouveau garage partenaire</p>
</div>

<div class="bg-white rounded-xl shadow-lg p-6 md:p-8 max-w-2xl">
    <form method="POST" action="{{ route('garages.store') }}" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="text" name="nom" value="{{ old('nom') }}" 
                ...
                placeholder="Ex: Garage Auto Express" required>
            @error('nom')
        ...
        <div>
            ...
            <input type="text" name="adresse" value="{{ old('adresse') }}" 
                ...
                placeholder="Ex: 123 Rue de la République, 75001 Paris" required>
            @error('adresse')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
